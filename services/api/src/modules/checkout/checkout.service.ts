...
export async function confirmHostedCheckout(input: {
  ...
  const paymentMethodProvider = resolveProviderFromPaymentMethod(input.paymentMethod);
  ...
  if (chargeQueue) {
    await safeQueueAdd("charge-queue", () =>
      chargeQueue.add(
        "execute-charge",
        {
          transactionId: transaction.id,
...
export async function executeAsynchronousCharge(input: {
        ...
        async (tx) => {
          ...
          await tx.transaction.update({
            ...
            data: {
              ...
              failureReason: result.status === "FAILED" ? "Payment authorization failed" : null
            }
          });
  ...
  if (nextStatus === "SUCCEEDED" || nextStatus === "FAILED") {
    ...
    await maybeFinalizeCreditPurchaseFromTransaction({
      ...
      metadata: transaction.metadata,
      settlementAmount: transaction.settlementAmount,
      failureReason: nextStatus === "FAILED" ? "Payment authorization failed" : null
    });
  }
